...
with col2:
    ...
    for i, faq in enumerate(SAMPLE_FAQS):
        with cols[i]:
            if st.button(faq, key=f"faq_{i}_{st.session_state.current_conversation_id}", help=f"{faq}에 대해 질문하기"):
                current_conv["messages"].append(
                    {"role": "user", "content": faq, "timestamp": datetime.now().strftime("%H:%M")})
                st.session_state.is_typing = True
                st.rerun()

    st.markdown("**📷 이미지 업로드 및 질문**")
    with st.form(key=f"chat_form_{st.session_state.current_conversation_id}", clear_on_submit=True):
        user_input = st.text_input("메시지를 입력하세요...", placeholder="세탁기/건조기에 대해 궁금한 점을 물어보세요!",
                                   label_visibility="collapsed")
        uploaded_image = st.file_uploader("이미지 첨부 (선택 사항)", type=["jpg", "jpeg", "png"],
...
with col3:
    ...
    if current_conv["image"]:
        ...
        st.image(current_conv["image"], caption="업로드된 이미지", use_container_width=True)
        try:
...
if send_button:
    ...
    if uploaded_image is not None:
        ...
        if not user_input:
            current_conv["messages"].append(
                {"role": "user", "content": "이미지를 업로드했습니다.", "timestamp": datetime.now().strftime("%H:%M")})
...
if st.session_state.is_typing:
    ...
    current_conv["messages"].append(
        {"role": "assistant", "content": bot_response, "timestamp": datetime.now().strftime("%H:%M")})
    st.session_state.is_typing = False
    st.rerun()
...
with st.sidebar:
    ...
    if st.button("📁 대화 기록 저장"):
        try:
            chat_history = {"timestamp": datetime.now().isoformat(), "conversations": st.session_state.conversations}
            st.download_button(
                label="💾 JSON 다운로드",
